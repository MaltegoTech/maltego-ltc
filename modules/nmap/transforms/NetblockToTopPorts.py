...
@nmap_registry.register_transform(display_name="To Top Ports (--top-ports -A)", input_entity="maltego.CIDR",
...
class NetblockToTopPorts(DiscoverableTransform):
    CMD = "{target} --top-ports {top_ports_scan_number} -A"
    @classmethod
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        netblock_cidr = request.Value
        parsing_function = COMMANDS[cls.CMD]
        cmd = cls.CMD.format(target=netblock_cidr, top_ports_scan_number=TOP_PORT_SCAN_NUMBER)
